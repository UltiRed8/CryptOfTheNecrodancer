...
void MenuManager::InitMenuPause()
{
	new ActionMap("GamePaused",
		{ ActionData("Escape", [this]()
			{ MenuManager::GetInstance().Get("GamePause")->Toggle(); MusicManager::GetInstance().Toggle(); },
	...
	function<void()> _callbackContinue = [this]() { MenuManager::GetInstance().Get("GamePause")->Toggle(); MusicManager::GetInstance().Toggle(); };
	...
	new Menu("GamePause", { new UIImage(Vector2f(0.f,0.f), Vector2f((float)window->getSize().x, (float)window->getSize().y), "PauseMenu.png"),
		...
		new UIButton(Vector2f(static_cast<float>(window->getSize().x / 2), static_cast<float>(window->getSize().y / 2.2)), Color::White, Color::Cyan, "Return to Lobby", 50, "Assets/Font/Font.ttf", _callbackLobby),
...
void MenuManager::InitMenuOptions()
{
	...
	new Menu("Options", { new UIImage(Vector2f(0.f,0.f), Vector2f((float)window->getSize().x, (float)window->getSize().y), "OptionsMenu.png"),
		new UIText(Vector2f(static_cast<float>(window->getSize().x / 2), static_cast<float>(window->getSize().y / 1.8)), Color::White, "Music Volume",50,"Assets/Font/Font.ttf"),
		new ProgressBar(PT_LEFT, Vector2f(static_cast<float>(window->getSize().x / 2.9), static_cast<float>(window->getSize().y / 1.6)), Vector2f(400.0f, 30.0f), "EmptyBar.png", "FullBar.png"),
		new UIButton(Vector2f(static_cast<float>(window->getSize().x / 1.45), static_cast<float>(window->getSize().y / 1.65)), Color::White, Color::Cyan, ">", 60, "Assets/Font/Font.ttf", _volumeUp),
		new UIButton(Vector2f(static_cast<float>(window->getSize().x / 3.2), static_cast<float>(window->getSize().y / 1.63)), Color::White, Color::Cyan, "<", 60, "Assets/Font/Font.ttf", _volumeDown),
		new UIButton(Vector2f(static_cast<float>(window->getSize().x / 2), static_cast<float>(window->getSize().y / 1.3)), Color::White, Color::Cyan, "Exit Options", 50, "Assets/Font/Font.ttf", _close) }, 2);
}
